...
(function (root) {
  ...
  function renderCard(card, style, w, h) {
    ...
    var lines = card.lines.map(function (l) {
      var txt = p.upper ? l.text.toUpperCase() : l.text;
      var size = base * (l.emphasis ? emph : 1);
      g.font = p.weight + ' ' + size + 'px ' + p.font;
      var tw = g.measureText(txt).width * (1 + p.letter);
      if (tw > maxW) { size *= maxW / tw; tw = maxW; }
      return { text: txt, size: size, width: tw, emphasis: l.emphasis };
    });
    // Empilé : interligne serré (comme la référence)
    var lh = 0.92, total = 0;
    lines.forEach(function (l) { total += l.size * lh; });
    var cy = h * (style.y == null ? 0.5 : style.y);
    var y = cy - total / 2;
    g.textAlign = 'center'; g.textBaseline = 'alphabetic';
    lines.forEach(function (l) {
      g.font = p.weight + ' ' + l.size + 'px ' + p.font;
      try { g.letterSpacing = (p.letter * l.size) + 'px'; } catch (e) {}
      var by = y + l.size * lh * 0.8;
      if (p.shadow) {
        g.save();
        g.shadowColor = 'rgba(0,0,0,' + p.shadow + ')'; g.shadowBlur = l.size * 0.25; g.shadowOffsetY = l.size * 0.04;
        g.fillStyle = p.color; g.fillText(l.text, w / 2, by);
        g.restore();
      }
      if (p.stroke) { g.lineWidth = l.size * 0.08 * p.stroke; g.strokeStyle = 'rgba(0,0,0,.9)'; g.lineJoin = 'round'; g.strokeText(l.text, w / 2, by); }
      g.fillStyle = p.color; g.fillText(l.text, w / 2, by);
      y += l.size * lh;
    });
    return cv;
